...
Vector2d evaluateCubicBezier_deCasteljau(const std::vector<Vector2d> &aCPs, float t)
{
	...
	for (unsigned int i = 0; i < aCPs.size() - 1; ++i)
...
int main()
{
	...
	WindowManager::Instance()->listen(&onKeyboardEvent);
    ...
    {	
		...
		// 
		BatchGeometryPtr pPolylineBatchGeometry = system.drawPolyline( polyline, Color(255, 0, 0) ); 
		BatchGeometryPtr pCurveBatchGeometry	= system.drawPolyline( curvePolyline, Color(0, 255, 0) ); 
    }
